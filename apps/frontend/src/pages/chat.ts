...
import { chatState, notify, appendDMSystemMessage } from "./chat/state";
import type {
  ChatMode,
  ConversationPreview,
  DisplayMessage,
} from "./chat/types";
import { showProfilePreviewModal, type ProfilePreviewHandle } from "../components/ProfilePreviewModal";
...
const attemptedSummaryFetch = new Set<string>();
let unsubscribeFromMessages: (() => void) | null = null;
let unsubscribeFromOpen: (() => void) | null = null;

async function openUserProfile(userId: string | null | undefined): Promise<void> {
  const targetId = typeof userId === 'string' && userId.length > 0 ? userId : null;
  if (!targetId) {
    showError('View Profile', 'Unable to open this profile right now.');
    return;
  }

  let previewHandle: ProfilePreviewHandle | null = null;
  try {
    previewHandle = await showProfilePreviewModal({
      userId: targetId,
      onClose: () => {
        previewHandle = null;
      },
      onOpenDM: (id, profile) => {
        previewHandle?.close();
        void switchToDM(id, profile.displayName, {
    ...
    console.error('Failed to open profile preview:', error);
    showError(
      'View Profile',
      error instanceof Error ? error.message : 'Unable to open this profile right now.'
    );
  }
}
...
async function loadChannels(): Promise<void> {
  ...
  try {
    ...
    chatState.channels = Array.isArray(channels) ? channels : [];
...
function handleWebSocketMessage(message: WSChatMessage): void {
  const viewerId = getUserId();
  ...
  if (message.type === "channel" && message.room === chatState.currentChannel) {
    ...
    const existingSummary = userSummaryCache.get(senderId);
    if (message.displayName) {
      userSummaryCache.set(senderId, {
        displayName: message.displayName,
        avatarUrl: existingSummary?.avatarUrl ?? null,
      });
    }

    const summary = userSummaryCache.get(senderId) ?? existingSummary;
    const displayName =
      message.displayName || summary?.displayName || getFallbackDisplayName(senderId);
    const avatarUrl = summary?.avatarUrl ?? null;
    const messageId = String(message.ts ?? `${senderId}-${timestamp}`);

    const isDuplicate = chatState.messages.some(
      (existing) =>
    ...
    const shouldFetchSummary =
      (!summary || summary.avatarUrl === null) && !attemptedSummaryFetch.has(senderId);

    if (shouldFetchSummary && !pendingSummaryFetches.has(senderId)) {
      pendingSummaryFetches.add(senderId);
      attemptedSummaryFetch.add(senderId);
      void resolveUserSummary(senderId).then((resolved) => {
        const target = chatState.messages.find(
          (entry) =>
            entry.userId === senderId &&
            entry.timestamp === timestamp &&
            entry.content === content,
        );
        if (target && target.avatarUrl !== resolved.avatarUrl) {
          target.avatarUrl = resolved.avatarUrl;
          notify.messages();
        }
      }).finally(() => {
        pendingSummaryFetches.delete(senderId);
      });
    }
...
function initializeWebSocket(): void {
  ...
  if (!unsubscribeFromMessages) {
    unsubscribeFromMessages = chatWS.subscribe(handleWebSocketMessage);
  }

  if (!unsubscribeFromOpen) {
    unsubscribeFromOpen = chatWS.onOpen(() => {
...
export function createChatPage(): HTMLElement {
  ...
  const profileBtn = createButton(
    "",
    "px-3 py-2 flex items-center gap-2 rounded border border-[#00C8FF]/40 text-[#00C8FF] hover:border-[#00C8FF] transition-colors hidden"
  );
  const inviteBtn = createButton(
    "",
    "px-3 py-2 flex items-center gap-2 rounded border border-[#00C8FF]/40 text-[#00C8FF] hover:border-[#00C8FF] transition-colors hidden"
  );
  ...
  appendChildren(headerRight, [profileBtn, inviteBtn, blockBtn]);
  ...
  function updateProfileButton(conversation: ConversationPreview | null): void {
    if (!conversation) {
    ...
    profileBtn.innerHTML = "";
    const icon = createIcon("user", "h-4 w-4");
    const label = createElement("span", "text-sm");
    label.textContent = "View Profile";
    appendChildren(profileBtn, [icon, label]);
    profileBtn.onclick = () => {
      void openUserProfile(conversation.userId);
    };
  }

  function updateInviteButton(conversation: ConversationPreview | null): void {
    if (!conversation || chatState.blockedUsers.has(conversation.userId)) {
      inviteBtn.classList.add("hidden");
      inviteBtn.innerHTML = "";
      inviteBtn.onclick = null;
      inviteBtn.dataset.loading = "false";
      inviteBtn.disabled = false;
      return;
    }

    const isLoading = inviteBtn.dataset.loading === "true";
    inviteBtn.classList.remove("hidden");
    inviteBtn.innerHTML = "";
    inviteBtn.disabled = isLoading;

    const icon = createIcon("play", `h-4 w-4 ${isLoading ? "animate-pulse" : ""}`);
    const label = createElement("span", "text-sm");
    label.textContent = isLoading ? "Inviting..." : "Invite to Match";
    appendChildren(inviteBtn, [icon, label]);

    inviteBtn.onclick = async () => {
      if (inviteBtn.dataset.loading === "true") {
        return;
      }

      inviteBtn.dataset.loading = "true";
      inviteBtn.innerHTML = "";
      inviteBtn.disabled = true;
      const busyIcon = createIcon("play", "h-4 w-4 animate-pulse");
      const busyLabel = createElement("span", "text-sm");
      busyLabel.textContent = "Inviting...";
      appendChildren(inviteBtn, [busyIcon, busyLabel]);
      inviteBtn.onclick = null;

      try {
        await invitationManager.sendInvite(conversation.userId);
        showSuccess("Match Invite", `Invite sent to ${conversation.displayName}.`);
        userSummaryCache.set(conversation.userId, {
          displayName: conversation.displayName,
          avatarUrl: conversation.avatarUrl ?? null,
        });
        appendDMSystemMessage(conversation.userId, `You invited ${conversation.displayName} to a match.`, {
          displayName: conversation.displayName,
          avatarUrl: conversation.avatarUrl ?? null,
        });
      } catch (error) {
        ...
        const message =
          error instanceof Error && error.message.trim().length > 0
            ? error.message
            : "Unable to send match invitation right now.";
        showError("Match Invite", message);
      } finally {
        inviteBtn.dataset.loading = "false";
        updateInviteButton(conversation);
      }
    };
  }
  ...
  function updateHeader(): void {
    if (chatState.chatMode === "channel") {
      ...
      updateInviteButton(conversation);
      updateProfileButton(conversation);
    }
  }
  ...
  function renderMessages(): void {
    ...
    chatState.messages.forEach((msg) => {
      ...
      let avatarUrl: string | null = msg.avatarUrl ?? null;

      if (!avatarUrl) {
        const cached = userSummaryCache.get(msg.userId);
        if (cached?.avatarUrl) {
          avatarUrl = cached.avatarUrl;
        }
      }

      if (!avatarUrl && chatState.chatMode === "dm") {
        const conversation = getActiveConversation();
