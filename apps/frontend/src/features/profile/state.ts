...
export class ProfileStateManager {
   ...
   */
  async loadAll(userId: string, options: LoadAllOptions = {}): Promise<void> {
    const includeSecurity = options.includeSecurity ?? true;
    const tasks: Array<Promise<void>> = [
      this.loadProfile(userId),
      this.loadStats(userId, { refresh: false, limit: 10 }),
      this.loadGlobalRank(userId),
    ];

    if (includeSecurity) {
      tasks.push(this.loadTwoFactorStatus());
    } else {
      this.setState({
        twoFactor: null,
        twoFactorEnrollment: null,
  ...
  async refreshStats(userId: string): Promise<void> {
	await this.loadStats(userId, { refresh: false, limit: 10 });
