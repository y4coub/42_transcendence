...
type RematchRequestCallback = (message: RematchRequestServerMessage) => void;
type RematchAcceptCallback = (message: RematchAcceptServerMessage) => void;
type RematchDeclineCallback = (message: RematchDeclineServerMessage) => void;
...
export class WSMatchClient {
	...
	private rematchRequestCallbacks: Set<RematchRequestCallback> = new Set();
	private rematchAcceptCallbacks: Set<RematchAcceptCallback> = new Set();
	private rematchDeclineCallbacks: Set<RematchDeclineCallback> = new Set();
	...
	sendRematchAccept(): void {
		...
		const message: RematchAcceptMessage = {
			type: 'rematch_accept',
			matchId: this.matchId,
		};
...
\t * Decline rematch request
	 */
	sendRematchDecline(reason?: 'decline' | 'timeout' | 'disconnect'): void {
		if (!this.wsClient) {
			return;
		}

		const message: RematchDeclineMessage = {
	...
	onRematchRequest(callback: RematchRequestCallback): () => void {
		this.rematchRequestCallbacks.add(callback);
		return () => this.rematchRequestCallbacks.delete(callback);
	}

	onRematchAccept(callback: RematchAcceptCallback): () => void {
		this.rematchAcceptCallbacks.add(callback);
		return () => this.rematchAcceptCallbacks.delete(callback);
	}

	onRematchDecline(callback: RematchDeclineCallback): () => void {
		this.rematchDeclineCallbacks.add(callback);
		return () => this.rematchDeclineCallbacks.delete(callback);
	}
	...
	private handleMessage(message: unknown): void {
		...
		case 'rematch_request':
			this.rematchRequestCallbacks.forEach(cb => cb(msg as RematchRequestServerMessage));
			break;

		case 'rematch_accept':
			this.rematchAcceptCallbacks.forEach(cb => cb(msg as RematchAcceptServerMessage));
			break;

		case 'rematch_decline':
			this.rematchDeclineCallbacks.forEach(cb => cb(msg as RematchDeclineServerMessage));
			break;
