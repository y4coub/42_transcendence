...
export type OverlayType = 
	...
	| { type: 'rematch_prompt'; fromName: string; onAccept: () => void; onDecline: () => void }
	| { type: 'none' };

export class GameOverlay {
	private container: HTMLElement;
	private overlayElement: HTMLElement | null = null;
	private gameOverReasonEl: HTMLElement | null = null;
	...
	show(overlay: OverlayType): void {
		...
		switch (overlay.type) {
			...
			case 'rematch_prompt':
				this.overlayElement.classList.remove('pointer-events-none');
				this.overlayElement.classList.add('pointer-events-auto');
				this.renderRematchPrompt(content, overlay.fromName, overlay.onAccept, overlay.onDecline);
				break;
	...
	updateGameOverReason(message: string): void {
		if (this.gameOverReasonEl) {
			this.gameOverReasonEl.textContent = message;
		}
	}
	...
	private renderPaused(container: HTMLElement, pausedByName: string): void {
		...
		panel.className = 'flex max-w-sm flex-col items-center gap-4 rounded border border-[#00C8FF]/20 bg-[#1a1a24] px-8 py-10 text-center';
	...
	private renderWaitingOpponent(container: HTMLElement, onPlayBot?: () => void): void {
		const panel = document.createElement('div');
		panel.className = 'flex max-w-md flex-col items-center gap-5 rounded border border-[#00C8FF]/20 bg-[#1a1a24] px-8 py-10 text-center';
	...
	private renderRematchPrompt(container: HTMLElement, fromName: string, onAccept: () => void, onDecline: () => void): void {
		const panel = document.createElement('div');
		panel.className = 'flex max-w-md flex-col items-center gap-5 rounded border border-[#00C8FF]/20 bg-[#1a1a24] px-8 py-10 text-center';

		const title = document.createElement('div');
		title.className = 'text-2xl font-semibold text-[#E0E0E0]';
		title.textContent = `${fromName} wants a rematch`; 

		const message = document.createElement('div');
		message.className = 'text-sm text-[#E0E0E0]/70';
		message.textContent = 'Do you want to play another match together?';
		...
		acceptButton.className = 'inline-flex w-full items-center justify-center rounded border border-[#00C8FF]/50 bg-[#121216] px-5 py-3 text-sm font-semibold uppercase tracking-wide text-[#00C8FF] hover:bg-[#1a2432]';
		...
		declineButton.className = 'inline-flex w-full items-center justify-center rounded border border-[#FF008C]/50 bg-[#1b1425] px-5 py-3 text-sm font-semibold uppercase tracking-wide text-[#FF008C] hover:bg-[#24172e]';
	...
	private renderGameOver(
		...
		const panel = document.createElement('div');
		panel.className = 'flex max-w-md flex-col items-center gap-6 rounded border border-[#00C8FF]/20 bg-[#1a1a24] px-8 py-10 text-center';
		...
		scoreBox.className = 'flex items-center justify-center gap-4 rounded border border-[#00C8FF]/25 bg-[#121321] px-6 py-4 text-3xl font-bold text-[#00C8FF]';
		...
		reasonText.className = 'text-sm text-[#E0E0E0]/60 whitespace-pre-line';
		let reasonMessage = 'Thanks for playing!';
		if (reason === 'forfeit') {
			reasonMessage = 'Match ended by forfeit.';
		} else if (reason === 'disconnect') {
			reasonMessage = 'Match ended due to a disconnect.';
		} else if (reason && reason !== 'score') {
			reasonMessage = reason;
		}
		reasonText.textContent = reasonMessage;
		this.gameOverReasonEl = reasonText;
		...
		playAgainButton.className = 'inline-flex w-full items-center justify-center rounded border border-[#00C8FF]/50 bg-[#121216] px-5 py-3 text-sm font-semibold uppercase tracking-wide text-[#00C8FF] hover:bg-[#1a2432]';
