export async function run(ctx) {
	...
	await ctx.step('matches', 'create-tournament', async () => {
	...
	await ctx.step('matches', 'list-tournaments', async () => {
	...
	await ctx.step('matches', 'register-players', async () => {
	...
	await ctx.step('matches', 'create-match', async () => {
		const tournamentId = ctx.state.tournament.id;
	...
	await ctx.step('matches', 'get-match-detail', async () => {
	...
	await ctx.step('matches', 'record-match-result', async () => {
		const matchId = ctx.state.tournament.matchId;
		const players = ctx.state.tournament.players;
	...
	await ctx.step('matches', 'verify-match-updated', async () => {
		const matchId = ctx.state.tournament.matchId;
		const response = await ctx.request('GET', `/matches/${matchId}`, {
			token,
			expectedStatus: 200,
	...
	await ctx.step('matches', 'tournament-board', async () => {
		const tournamentId = ctx.state.tournament.id;
