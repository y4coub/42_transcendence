...
/**
 * Find latest pending (waiting/countdown) match between two players
 */
export function findPendingMatchBetween(playerA: string, playerB: string): MatchRecord | null {
	const db = getDatabase();
	...
	return stmt.get(playerA, playerB, playerB, playerA) as MatchRecord | undefined ?? null;
}
