...
export const fetchProfile = async (accessToken: string): Promise<OAuth42Profile> => {
  ...
  const preferredName = payload.displayname ?? payload.usual_full_name ?? '';

  return {
    id: payload.id.toString(),
    email: payload.email ?? '',
    displayName: login || preferredName || payload.id.toString(),
    username: login || payload.id.toString(),
    fullName: preferredName || null,
