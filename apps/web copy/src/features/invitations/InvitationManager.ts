...
import { showError, showSuccess } from '../../components/Modal';
...
export class InvitationManager {
  ...
  private handleInviteAcknowledged(message: ChatMessage): void {
    ...
    this.getProfile(toId).then((profile) => {
      ...
      showSuccess('Invitation Sent', `Waiting for ${profile.displayName} to respond.`);
    }).catch(() => {
      this.outgoingInvites.set(inviteId, {
        inviteId,
        opponentId: toId,
      ...
      showSuccess('Invitation Sent', 'Waiting for opponent to respond.');
    });
  ...
  private handleInviteCancelled(message: ChatMessage): void {
    const inviteId = String(message.inviteId ?? '');
    if (this.incomingInvites.has(inviteId)) {
  ...
  private handleInviteAccepted(message: ChatMessage, inviterPerspective: boolean): void {
    ...
    if (inviterPerspective) {
      ...
      if (incoming) {
        opponentName = incoming.fromName;
        this.incomingInvites.delete(inviteId);
      }
      closeIncomingInviteModal();
    }

    this.launchMatch(matchId, opponentId, opponentName);
  }

  private launchMatch(matchId: string, opponentId: string, opponentName: string): void {
    ...
    showSuccess('Match Starting', `Loading arena against ${opponentName}...`);
    appState.setState({ currentPage: 'game' });
  }
