 ...
 * - Match chat sidebar (populated by ChatPanel.ts)
...
export function updatePlayerInfo(
  ...
  if (avatarContainer) {
    ...
    avatarContainer.appendChild(createAvatar(displayName, avatarUrl, "h-12 w-12", borderColor));
  }
}
...
export function createGamePage(): HTMLElement {
  ...
  statusRow.appendChild(createInfoPill("match-round", "Round", "Best of 3"));
  ...
  const gameShell = createDiv("overflow-hidden rounded-2xl border border-[#00C8FF]/20 bg-[#1a1a24]");
  ...
  player1Avatar.appendChild(createAvatar("Loading...", null, "h-14 w-14", "border-[#00C8FF]"));
  ...
  const player1Score = createBadge("0", "border-[#00C8FF]/40 bg-[#00C8FF]/10 text-[#00C8FF]");
  ...
  const scoreStack = createDiv("flex flex-col items-center gap-1 rounded-xl border border-[#00C8FF]/20 bg-[#121321]/80 px-5 py-3");
  const scoreTitle = createElement("span", "text-[10px] uppercase tracking-[0.5em] text-[#E0E0E0]/40");
  scoreTitle.textContent = "Scoreboard";
  const scoreValue = createElement("p", "text-4xl font-bold text-[#00C8FF] transition-transform duration-200 lg:text-5xl");
  scoreValue.id = "match-score";
  ...
  const player2Score = createBadge("0", "border-[#FF008C]/40 bg-[#FF008C]/10 text-[#FF008C]");
  ...
  player2Avatar.appendChild(createAvatar("...", null, "h-14 w-14", "border-[#FF008C]"));
  ...
  appendChildren(canvasWrap, [canvas, overlayContainer]);
  ...
  const helperGrid = createDiv("grid grid-cols-1 gap-4 text-sm md:grid-cols-2");

  const howToCard = createDiv("space-y-3 rounded-2xl border border-[#00C8FF]/15 bg-[#1a1a24] p-6");
  const howToTitle = createElement("h3", "text-xs uppercase tracking-[0.35em] text-[#E0E0E0]/60");
  howToTitle.textContent = "Match Flow";
  const howToList = createElement("ul", "space-y-2 text-[#E0E0E0]/70");
  howToList.innerHTML = `
    <li>• Ready up when both players join.</li>
    <li>• First to 11 points wins the round.</li>
    <li>• Pause requests are limited to 2 per match.</li>
    <li>• Disconnecting forfeits after 30 seconds.</li>
  `;
  appendChildren(howToCard, [howToTitle, howToList]);

  const controlsCard = createDiv("space-y-3 rounded-2xl border border-[#00C8FF]/15 bg-[#1a1a24] p-6");
  const controlsTitle = createElement("h3", "text-xs uppercase tracking-[0.35em] text-[#E0E0E0]/60");
  controlsTitle.textContent = "Controls";
  const controlGrid = createDiv("grid grid-cols-1 gap-3 text-[#E0E0E0]/70 sm:grid-cols-2");
  controlGrid.innerHTML = `
    <div class="rounded-xl border border-[#00C8FF]/25 bg-[#121425]/80 p-4">
    ...
    <div class="rounded-xl border border-[#FF008C]/25 bg-[#1b1425]/80 p-4">
      <p class="mb-1 text-[11px] uppercase tracking-wide text-[#FF008C]">Mouse</p>
      <p class="text-sm text-[#E0E0E0]/80">Move cursor to guide paddle</p>
    </div>
    <div class="col-span-1 rounded-xl border border-[#00C8FF]/20 bg-[#121425]/80 p-4 sm:col-span-2">
  ...
  appendChildren(helperGrid, [howToCard, controlsCard]);

  appendChildren(mainArea, [gameShell, helperGrid]);
  ...
  const matchDetailsCard = createDiv("overflow-hidden rounded-2xl border border-[#00C8FF]/15 bg-[#1a1a24]");
  ...
  timeline.innerHTML = `
    <li class="flex gap-3">
      ...
      <div>
        <p class="font-medium text-[#E0E0E0]">Warmup</p>
        <p class="text-sm text-[#E0E0E0]/60">Practice against bot while we locate your opponent.</p>
      </div>
    </li>
    <li class="flex gap-3">
      <span class="mt-1 h-2 w-2 rounded-full bg-[#00C8FF]/70"></span>
      <div>
        <p class="font-medium text-[#E0E0E0]">Countdown</p>
        <p class="text-sm text-[#E0E0E0]/60">Both players ready? Countdown begins from three.</p>
      </div>
    </li>
    <li class="flex gap-3">
      <span class="mt-1 h-2 w-2 rounded-full bg-[#FF008C]"></span>
      <div>
        <p class="font-medium text-[#E0E0E0]">Game Live</p>
        <p class="text-sm text-[#E0E0E0]/60">Track latency and score in real time. Good luck!</p>
      </div>
    </li>
  `;
  ...
  const chatCard = createDiv("flex h-[620px] flex-col overflow-hidden rounded-2xl border border-[#00C8FF]/20 bg-[#1a1a24]");
  chatCard.id = "game-chat";

  const chatHeader = createDiv("flex items-center justify-between border-b border-[#00C8FF]/15 px-5 py-4");
  const chatTitle = createElement("h3", "text-sm font-semibold uppercase tracking-wide text-[#E0E0E0]");
  chatTitle.textContent = "Match Chat";
  const chatHint = createElement("span", "text-[11px] uppercase tracking-[0.3em] text-[#E0E0E0]/40");
  chatHint.textContent = "Team Safe";
  appendChildren(chatHeader, [chatTitle, chatHint]);

  const chatPlaceholder = createDiv("flex flex-1 flex-col items-center justify-center gap-4 px-5 py-6 text-center");
  const chatIcon = createElement("div", "flex h-14 w-14 items-center justify-center rounded-full border border-dashed border-[#00C8FF]/40 text-2xl text-[#00C8FF]/70");
  chatIcon.textContent = "💬";
  const chatMessage = createElement("p", "max-w-xs text-sm text-[#E0E0E0]/60");
  chatMessage.textContent = "Chat unlocks automatically once both players are connected. Stay respectful, stay focused.";
  chatPlaceholder.appendChild(chatIcon);
  chatPlaceholder.appendChild(chatMessage);

  appendChildren(chatCard, [chatHeader, chatPlaceholder]);
