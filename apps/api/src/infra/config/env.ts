...
const configSchema = z.object({
  ...
  DOCS_BASIC_AUTH_USER: z.string().min(1).optional(),
  DOCS_BASIC_AUTH_PASSWORD: z.string().min(1).optional(),
...
if ((env.DOCS_BASIC_AUTH_USER && !env.DOCS_BASIC_AUTH_PASSWORD) || (!env.DOCS_BASIC_AUTH_USER && env.DOCS_BASIC_AUTH_PASSWORD)) {
  throw new Error('DOCS_BASIC_AUTH_USER and DOCS_BASIC_AUTH_PASSWORD must be provided together');
}
...
export const config = {
  ...
  docs: {
    basicAuth:
      env.DOCS_BASIC_AUTH_USER && env.DOCS_BASIC_AUTH_PASSWORD
        ? {
            username: env.DOCS_BASIC_AUTH_USER,
